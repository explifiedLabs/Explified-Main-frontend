  ...
  const icons = {
    AWS: (
      <svg viewBox="0 0 24 24" fill="none" className={className}>
        <path d="M17.5 18C17.5 18 15 20 12 20C8 20 5 17 5 17" stroke="#FF9900" strokeWidth="2" strokeLinecap="round" />
        <path d="M6 10L9 5L12 10L15 5L18 10" stroke="#FF9900" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    ),
    Snowflake: (
      <svg viewBox="0 0 24 24" fill="none" className={className}>
...
const Navbar = () => {
    ...
    const hasContent = (key) => {
      ...
      return (
        Object.entries(val).some(
          ([k, v]) => k !== "url" && k !== "openInNewTab" && v && typeof v === "object" && !Array.isArray(v)
  ...
  return (
    ...
    >
              ...
              <div key={menuKey} className="relative z-50" onMouseEnter={() => (hasContent ? setHoveredMenu(menuKey) : null)}>
                ...
                {useSideTabUI ? (
                  ...
                  <div
                    ...
                    style={{ width: resolvedGroups.length <= 2 ? "700px" : "1000px" }}
                  >
                    <div
                      ...
                      style={{ gridTemplateColumns: `repeat(${resolvedGroups.length}, minmax(0, 1fr))` }}
                    >
                      {resolvedGroups.map((group) => (
                        <div key={group.title}>
            ...
            return (
              <div key={menuKey} className="border-b border-white/5 pb-3">
                <div className="w-full flex items-center justify-between py-4">
                  <span onClick={() => handleCategoryClick(menuKey)} className={`text-2xl font-bold text-white ${hasUrl ? "cursor-pointer hover:text-[#23b5b5]" : ""}`}>
                    ...
                    {resolvedGroups.map((group) => (
                      <div key={group.title}>
                        ...
                        <div className="space-y-8">
                          {group.items.map((item, idx) => (
                            <a key={idx} href={item.url || "#"} target={item.openInNewTab ? "_blank" : "_self"} rel="noopener noreferrer" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-5 group active:opacity-70 transition-opacity">
      ...
      {/* AUTH MODAL */}
      <div className={`fixed inset-0 z-[200] flex items-center justify-center px-6 transition-all duration-300 pointer-events-auto ${authLoading ? "cursor-not-allowed" : ""} ${isAuthModalOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}>
        <div onClick={() => setIsAuthModalOpen(false)} className="absolute inset-0 bg-black/70 backdrop-blur-md" />

        <div className={`relative w-full max-w-xl rounded-[32px] border border-white/10 bg-[#080B12]/95 backdrop-blur-3xl shadow-[0_0_80px_rgba(35,181,181,0.08)] overflow-hidden transition-all duration-300 ${isAuthModalOpen ? "scale-100 translate-y-0" : "scale-95 translate-y-6"}`}>
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(35,181,181,0.12),transparent_55%)] pointer-events-none" />

          <button onClick={() => setIsAuthModalOpen(false)} className="absolute top-5 right-5 z-10 w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white transition-all">
            <LucideIcons.X size={18} />
          </button>

          <div className="relative p-7 md:p-8">
            <div className="flex items-center rounded-2xl border border-white/10 bg-white/[0.03] p-1 mb-8">
              <button onClick={() => setAuthMode("signin")} className={`flex-1 h-11 rounded-xl text-xs font-black tracking-[0.15em] transition-all duration-300 ${authMode === "signin" ? "bg-[#23b5b5] text-black shadow-lg" : "text-gray-500 hover:text-white"}`}>SIGN IN</button>
              <button onClick={() => setAuthMode("signup")} className={`flex-1 h-11 rounded-xl text-xs font-black tracking-[0.15em] transition-all duration-300 ${authMode === "signup" ? "bg-[#23b5b5] text-black shadow-lg" : "text-gray-500 hover:text-white"}`}>SIGN UP</button>
            </div>

            <div className="mb-7">
              <h2 className="text-3xl font-black text-white tracking-tight">{authMode === "signin" ? "Welcome back" : "Create your account"}</h2>
              <p className="text-gray-500 mt-2 text-sm">{authMode === "signin" ? "Sign in to your automation workspace" : "Start automating in minutes — it's free"}</p>
            </div>

            <button onClick={handleGoogleLogin} disabled={authLoading} className={`w-full h-14 rounded-2xl border border-white/10 bg-white/[0.03] hover:bg-[#23b5b5]/10 hover:border-[#23b5b5]/30 transition-all duration-300 flex items-center justify-center gap-4 text-white font-semibold text-base ${authLoading ? "cursor-not-allowed opacity-70" : "cursor-pointer"}`}>
            ...
            <div className="flex items-center gap-4 my-7">
              ...
              <span className="text-[10px] font-bold tracking-[0.3em] text-gray-600">OR</span>
              <div className="flex-1 h-px bg-white/10" />
            </div>
            ...
            <div className="space-y-3">
              {authMode === "signup" && (
                <div className="h-14 rounded-2xl border border-white/10 bg-white/[0.03] px-5 flex items-center gap-4">
                  ...
                  <input type="text" name="name" value={authForm.name} onChange={handleInputChange} placeholder="Full name" className="bg-transparent outline-none w-full text-sm text-white placeholder:text-gray-500" />
                </div>
              )}

              <div className="h-14 rounded-2xl border border-white/10 bg-white/[0.03] px-5 flex items-center gap-4">
                <LucideIcons.Mail size={17} className="text-gray-500" />
                <input type="email" name="email" value={authForm.email} onChange={handleInputChange} placeholder="you@company.com" className="bg-transparent outline-none w-full text-sm text-white placeholder:text-gray-500" />
              </div>

              <div className="h-14 rounded-2xl border border-white/10 bg-white/[0.03] px-5 flex items-center gap-4">
                <LucideIcons.Lock size={17} className="text-gray-500" />
                <input type="password" name="password" value={authForm.password} onChange={handleInputChange} placeholder="Password" className="bg-transparent outline-none w-full text-sm text-white placeholder:text-gray-500" />
              </div>

              {authMode === "signup" && (
                <div className="h-14 rounded-2xl border border-white/10 bg-white/[0.03] px-5 flex items-center gap-4">
                  <LucideIcons.Lock size={17} className="text-gray-500" />
                  <input type="password" name="confirmPassword" value={authForm.confirmPassword} onChange={handleInputChange} placeholder="Confirm password" className="bg-transparent outline-none w-full text-sm text-white placeholder:text-gray-500" />
                </div>
              )}
            </div>

            <button onClick={handleAuthSubmit} disabled={authLoading} className={`w-full h-14 rounded-2xl bg-[#23b5b5] hover:bg-[#1da0a0] disabled:opacity-50 text-black text-base font-black mt-7 transition-all duration-300 hover:shadow-[0_0_30px_rgba(35,181,181,0.35)] ${authLoading ? "cursor-not-allowed" : "cursor-pointer"}`}>
              {authLoading ? "Please wait..." : authMode === "signin" ? "Sign In — It's Free" : "Create Account →"}
            </button>
              ...
              {authMode === "signin" ? (
                <>
                  <div className="text-gray-500">
                    No account? <button onClick={() => setAuthMode("signup")} className="text-[#23b5b5] font-bold hover:text-white transition-colors">Create one →</button>
                  </div>
                  <button className="text-gray-600 hover:text-gray-400 transition-colors">Forgot?</button>
                </>
              ) : (
                <div className="text-gray-500">
                  Already have an account? <button onClick={() => setAuthMode("signin")} className="text-[#23b5b5] font-bold hover:text-white transition-colors">Sign in →</button>
                </div>
              )}
            </div>
          </div>
