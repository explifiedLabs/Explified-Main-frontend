...
const RefundTerms = () => {
  ...
  useEffect(() => {
    const fetchRefund = async () => {
      try {
        const response = await fetch('https://cmsapi-pf6diz22ka-uc.a.run.app/api/pages/refund-terms',{
            headers: {
              "x-site-id": SITE_ID,
            },
          },);
        ...
        console.error("Failed to fetch Refund Terms:", error);
      } finally {
        setLoading(false);
      }
    };
      ...
      >
        ...
        <div className="space-y-8 text-base leading-relaxed">
          {data.content?.sections?.map((section, idx) => {
            const isOrdered = section.title.toLowerCase().includes('how to');
            const ListTag = isOrdered ? 'ol' : 'ul';
            const listClasses = isOrdered 
                ? "list-decimal pl-6 space-y-2 text-zinc-400 marker:text-teal-400" 
                : "list-disc pl-6 space-y-2 text-zinc-400 marker:text-teal-400";
