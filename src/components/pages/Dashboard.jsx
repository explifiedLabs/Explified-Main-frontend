...
// --- CONSTANTS ---
const DEFAULT_DESC =
  "Streamline your operations and unlock new capabilities with this powerful integration. Designed to seamlessly connect your workspace, automate repetitive tasks, and sync data in real-time, saving you countless hours of manual work.";
...
const StatsShimmer = () => (
  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 my-8">
    {[1, 2, 3].map((i) => (
      <div
        key={i}
        className="bg-[#18191E] border border-zinc-800 rounded-xl p-5 flex items-center gap-4 animate-pulse"
      >
...
const Lightbox = ({ images, startIndex, onClose }) => {
  ...
  return (
    ...
    >
      ...
      {images.length > 1 && (
        <button
          className="absolute left-5 z-10 w-12 h-12 flex items-center justify-center rounded-full bg-white text-black hover:bg-zinc-200 shadow-xl transition-colors"
          onClick={prev}
        >
          <ChevronLeft size={22} />
        </button>
      ...
      <div
        className="relative w-full h-full max-w-[1100px] max-h-[88vh] mx-auto flex items-center justify-center px-20"
      ...
      {images.length > 1 && (
        <button
          className="absolute right-5 z-10 w-12 h-12 flex items-center justify-center rounded-full bg-white text-black hover:bg-zinc-200 shadow-xl transition-colors"
          onClick={next}
        >
          ...
          {images.map((_, i) => (
            <button
              ...
              style={{
                ...
                borderRadius: 4,
                background:
                  i === current ? "#23b5b5" : "rgba(255,255,255,0.35)",
...
export default function FullDashboard() {
    ...
    const fetchProducts = async () => {
      ...
      try {
        const response = await fetch(
          "https://cmsapi-pf6diz22ka-uc.a.run.app/api/menus/products/info",
          {
        ...
        if (json?.data?.header?.products) {
          ...
          Object.keys(prodsObj).forEach((key) => {
            ...
            if (categoryNode && typeof categoryNode === "object") {
              ...
              const catIconUrl =
                categoryNode.imageUrl ||
                categoryNode.iconUrl ||
                categoryNode.icon ||
                null;
              const items = Array.isArray(categoryNode.items)
                ? categoryNode.items
                : [];
              const mappedItems = items.map((item, i) => {
                const appIconUrl =
                  item.iconUrl || item.icon || item.imageUrl || null;
                return {
                  ...
                  path:
                    item.url ||
                    `explified/${(item.title || item.label || "app").toLowerCase().replace(/\s+/g, "-")}`,
                  tag: item.tag || "",
                  iconType:
                    item.type ||
                    item.iconType ||
                    (appIconUrl ? "upload" : "lucide"),
                  iconName: item.iconName || "Box",
                  iconUrl: appIconUrl,
                  features:
                    Array.isArray(item.features) && item.features.length > 0
                      ? item.features
                      : ["One-click deployment", "High performance data sync"],
                  featureImages: Array.isArray(item.featureImages)
                    ? item.featureImages
                    : [],
                  pricingModels: Array.isArray(item.pricingModels)
                    ? item.pricingModels
                    : [],
              ...
              if (mappedItems.length > 0) {
                categories.push({
                  ...
                  iconType: catIconUrl
                    ? "upload"
                    : categoryNode.iconType || "lucide",
                  iconUrl: catIconUrl,
                  iconName:
                    categoryNode.iconName || getMarketplaceIcon(catName),
                  items: mappedItems,
                });
  ...
  const subscribedApps = useMemo(
    () => storeApps.filter((a) => a.subscribed),
    [storeApps],
  );
  const activeLogs = useMemo(
    () => subscriptionLogs.filter((l) => l.subscriptionStatus === "active"),
    [subscriptionLogs],
  );
  const cancelledLogs = useMemo(
    () => subscriptionLogs.filter((l) => l.subscriptionStatus === "cancelled"),
    [subscriptionLogs],
  );
  ...
  const handleCancelSubscription = async (appId, logId = null) => {
    ...
    try {
      ...
      setUserSubscriptions((prev) =>
        prev.map((s) =>
          s._id === subId ? { ...s, subscriptionStatus: "cancelled" } : s,
        ),
      );
  ...
  const SidebarLink = ({ id, name, icon: Icon, badge }) => {
    ...
    return (
      ...
      >
        ...
        <Icon
          size={16}
          className={`relative z-10 transition-colors ${isActive ? "text-[#23b5b5]" : "text-zinc-400 group-hover:text-zinc-200"}`}
        />
        <span
          className={`relative z-10 flex-1 text-left transition-colors ${isActive ? "text-white" : "text-zinc-400 group-hover:text-zinc-200"}`}
        >
          {name}
        </span>
        {badge > 0 && (
  ...
  const AppIcon = ({ app, size = "md" }) => {
    ...
    return (
      <div
        className={`${dim} rounded-lg flex-shrink-0 flex items-center justify-center bg-zinc-900 border border-zinc-700/50 overflow-hidden text-[#23b5b5]`}
      >
        {app.iconUrl ? (
          <img
            src={app.iconUrl}
  ...
  const ProductCard = ({ app }) => (
    <motion.div
      ...
      whileHover={{
        scale: 1.02,
        y: -4,
        boxShadow: "0 10px 40px -10px rgba(35,181,181,0.15)",
      }}
      whileTap={{ scale: 0.98 }}
    ...
    >
      <div className="flex gap-4 items-start mb-3">
        ...
        <div className="overflow-hidden">
          <h3 className="text-[15px] font-bold text-zinc-100 truncate">
            {app.name}
          </h3>
          {app.tag && (
            <p className="text-[11px] font-bold uppercase tracking-wider text-[#23b5b5] mt-0.5">
              {app.tag}
            </p>
          )}
          {app.subscribed && (
      ...
      <div className="flex items-center justify-between text-[13px] text-zinc-400 pt-3 border-t border-zinc-800/80 mt-auto">
        <div className="flex items-center gap-2">
          <div className="w-5 h-5 rounded bg-gradient-to-br from-[#23b5b5] to-[#167878] flex items-center justify-center text-[10px] font-bold text-white">
            E
          </div>
        ...
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1">
            <Star size={13} className="text-zinc-400" /> {app.rating}
          </span>
        </div>
      </div>
    </motion.div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div>
                ...
                <p className="text-[12px] text-zinc-500 font-mono mt-0.5 truncate max-w-[280px]">
                  {log.domain}
                </p>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3">
              <span className="flex items-center gap-1.5 text-[12px] text-zinc-400">
                <span className="capitalize px-1.5 py-0.5 bg-zinc-800 rounded text-zinc-300 text-[11px] font-medium">
                  {log.marketplace}
                </span>
              </span>
              <span className="flex items-center gap-1.5 text-[12px] text-zinc-400">
  ...
  const AppDetailView = () => {
    ...
    const matchingLog = subscriptionLogs.find(
      (l) =>
        l.domain.toLowerCase() === selectedApp.path.toLowerCase() &&
    ...
    return (
      <motion.div
        ...
        animate="animate"
        exit="exit"
        className="max-w-6xl mx-auto space-y-6 text-zinc-300 pb-20"
      >
        <div className="flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="flex-1">
            ...
            >
              ...
              {selectedApp.tag && (
                <span className="px-2 py-1 bg-[#23b5b5]/10 border border-[#23b5b5]/30 text-xs font-bold rounded text-[#23b5b5] uppercase tracking-wider">
                  {selectedApp.tag}
                </span>
              )}
            ...
            <motion.div
              ...
              className="flex flex-wrap items-center gap-4 text-[13px] text-zinc-400 mb-3"
            >
              ...
              <button
                ...
                onClick={() => {
                  navigator.clipboard.writeText(selectedApp.path);
              ...
              <span className="flex items-center gap-1">
                ...
                {selectedApp.rating} ({selectedApp.reviews})
              </span>
              <span className="flex items-center gap-1">
                <Users size={14} /> {selectedApp.installs}
              </span>
            </motion.div>
            ...
            <motion.p
              ...
              className="text-[14px] text-zinc-400 mt-4 mb-3 leading-relaxed max-w-3xl"
            >
              {selectedApp.desc}
            </motion.p>
            ...
            {selectedApp.subscribed && matchingLog && (
              ...
              >
                ...
                <span className="flex items-center gap-1.5 text-zinc-400">
                  ...
                  {matchingLog.planType} · $
                  {(matchingLog.amount || 0).toFixed(2)}
                </span>
              </motion.div>
            )}
            ...
            <motion.div
              ...
              animate={{ opacity: 1 }}
              transition={{ delay: 0.2 }}
              className="text-[13px] flex items-center flex-wrap gap-2 mt-3"
            >
              Crafted by{" "}
              <span className="text-[#23b5b5] font-medium inline-flex items-center gap-1">
                <div className="w-4 h-4 rounded bg-[#23b5b5] flex items-center justify-center text-[8px] text-black">
                  E
                </div>{" "}
                Explified
              </span>
              ...
              <span className="flex items-center gap-1.5 text-zinc-300 bg-zinc-800/50 px-2 py-0.5 rounded border border-zinc-700/50">
                {selectedApp.categoryIcon ? (
                  <img
                    src={selectedApp.categoryIcon}
                    className="w-3.5 h-3.5 object-contain"
                    alt={selectedApp.category}
                  />
                ) : (
                  <DynamicIcon
                    name={getMarketplaceIcon(selectedApp.category)}
                    size={12}
          ...
          <motion.div
            ...
            transition={{ delay: 0.1 }}
            className="flex flex-col items-stretch gap-2 w-full md:w-auto mt-2 md:mt-0"
          >
          ...
          {appDetailTab === "information" ? (
            ...
            >
              {images.length > 0 && (
                <div className="w-full">
                  {images.length === 1 && (
                    <div
                      className="w-full aspect-video md:aspect-[21/9] cursor-pointer rounded-2xl overflow-hidden border border-zinc-800/80 bg-[#18191E]"
                  ...
                  {images.length === 2 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {images.map((img, i) => (
                        <div
                          key={i}
                          className="cursor-pointer rounded-2xl overflow-hidden border border-zinc-800/80 aspect-video bg-[#18191E]"
                  ...
                  {images.length >= 3 && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-auto md:h-[450px]">
                      <div
                        className="md:col-span-2 h-[250px] md:h-full cursor-pointer rounded-2xl overflow-hidden relative group border border-zinc-800/80 bg-[#18191E]"
                        onClick={() => openLightbox(0)}
                      >
                        <img
                          src={images[0]}
                      ...
                      <div className="grid grid-rows-2 gap-4 h-[500px] md:h-full">
                        <div
                          className="cursor-pointer rounded-2xl overflow-hidden relative group border border-zinc-800/80 bg-[#18191E]"
              ...
              <div className="max-w-4xl space-y-6 bg-[#18191E] p-8 rounded-2xl border border-zinc-800/80 shadow-sm">
                <h2 className="text-2xl font-bold text-white">
                  {selectedApp.name} Setup & Capabilities
                </h2>
            ...
            <>
              ...
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="max-w-4xl bg-[#18191E] border border-zinc-800 rounded-xl p-6 mb-8"
              >
                <div className="flex items-center gap-3 mb-2">
                  <CreditCard size={20} className="text-[#23b5b5]" />
                ...
                <p className="text-sm text-zinc-400 mb-8">
                  Choose a plan for{" "}
                  <span className="font-mono text-zinc-300 bg-zinc-900 px-1 rounded">
                    {selectedApp.name}
                  </span>
                  .
                </p>

                {!selectedApp.pricingModels ||
                selectedApp.pricingModels.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    {selectedApp.pricingModels.map((plan) => (
                      <motion.div
                        key={plan._id || plan.planName}
                        whileHover={{ y: -4 }}
                        className="relative flex flex-col bg-zinc-950 border border-zinc-800 hover:border-[#23b5b5]/60 rounded-xl p-5 transition-colors"
                      >
                        {plan.badge && (
                          <span className="absolute -top-3 left-5 px-2.5 py-0.5 rounded-full bg-[#23b5b5] text-black text-[10px] font-bold uppercase tracking-wide">
                            {plan.badge}
                          </span>
                        )}
                        <h3 className="text-[15px] font-bold text-white mt-2">
                          {plan.planName}
                        </h3>
                          ...
                          <ul className="space-y-2.5 mb-6 flex-1">
                            {plan.features.map((f, i) => (
                              ...
                              >
                                <CheckCircle2
                                  size={14}
                                  className="text-[#23b5b5] mt-0.5 shrink-0"
                                />
                                <span>{f}</span>
                              </li>
                            ))}
                        ...
                        <motion.button
                          ...
                          whileTap={{ scale: 0.97 }}
                          onClick={() => initiateSubscription(selectedApp.id)}
                          className="mt-auto w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-[#23b5b5] hover:bg-[#1ca3a3] text-black text-sm font-semibold transition-colors"
                        >
                          {plan.ctaButtonIcon && (
                            <DynamicIcon name={plan.ctaButtonIcon} size={14} />
                          )}
              ...
              {/* existing API Credentials block stays exactly as-is below */}
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="max-w-4xl bg-[#18191E] border border-zinc-800 rounded-xl p-6"
              >
                {/* ...unchanged... */}
              </motion.div>
            </>
  ...
  const MarketplaceOverview = () => {
    ...
    const subscribedSet = new Set(
      storeApps.filter((a) => a.subscribed).map((a) => a.id),
    );
  ...
  const IntegrationsView = () => {
    const filteredLogs = useMemo(() => {
      if (logFilter === "active")
        return subscriptionLogs.filter(
          (l) => l.subscriptionStatus === "active",
        );
      if (logFilter === "cancelled")
        return subscriptionLogs.filter(
          (l) => l.subscriptionStatus === "cancelled",
        );
        ...
        <div className="flex items-start justify-between mb-2">
          <div>
            ...
            <p className="text-zinc-400 mt-1">
              All your subscriptions — active and past.
            </p>
          </div>
          <motion.button
        ...
        {logsLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 my-8">
            <div className="bg-[#18191E] border border-zinc-800 rounded-xl p-5 flex items-center gap-4">
              ...
              <div>
                <div className="text-xs text-zinc-400 font-medium uppercase tracking-wider mb-0.5">
                  Active
                </div>
        ...
        {logsLoading ? (
          ...
          >
            ...
            <p className="text-zinc-400 mt-2 text-sm max-w-sm mx-auto mb-6">
              You haven't subscribed to any products yet. Browse the store to
              get started.
            </p>
            <motion.button
              ...
              className="text-sm font-semibold bg-[#23b5b5] text-black px-6 py-2.5 rounded-lg hover:bg-[#1ca3a3] transition-colors"
            >
              Browse Store
            </motion.button>
          </motion.div>
          ...
          >
            {filteredLogs.map((log) => (
              <SubscriptionLogCard key={log._id} log={log} />
            ))}
          </motion.div>
        )}
      </motion.div>
  ...
  const BillingView = () => (
    ...
    >
      ...
      <p className="text-zinc-400 mb-8">
        Manage your subscription, compute usage, and billing history.
      </p>

      {userSubscriptions.filter((s) => s.subscriptionStatus === "active")
        .length > 0 && (
      ...
      <div className="bg-[#18191E] border border-zinc-800 rounded-xl p-6 text-center py-20">
        ...
        <p className="text-zinc-400 font-medium">
          Full billing module is integrated via Stripe in production.
        </p>
      </div>
    </motion.div>
  );
  ...
  const SettingsView = () => {
    ...
    const displayDomains = (user?.domains || []).filter(
      (d) => !d.startsWith("http") && d.length < 40,
    );
            ...
            {displayDomains.length > 0 && (
              <div className="flex flex-wrap gap-1.5 justify-center mt-3">
                {displayDomains.map((d) => (
                  <span
                    key={d}
                    className="text-[11px] px-2 py-0.5 bg-[#23b5b5]/10 text-[#23b5b5] border border-[#23b5b5]/20 rounded-full font-medium"
                  >
                    {d}
                  </span>
                ))}
  ...
  const ApiKeysView = () => {
    ...
    return (
      ...
      >
        ...
        <p className="text-zinc-400 mb-8">
          Centralized view of all your active access tokens.
        </p>
        ...
        {allKeys.length === 0 ? (
          ...
          >
            <p className="text-zinc-500">
              No active keys. Subscribe to a product first.
            </p>
          </motion.div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-zinc-800 bg-zinc-900/50">
                  <th className="p-4 text-xs font-semibold text-zinc-400 uppercase">
                    Product
                  </th>
                  <th className="p-4 text-xs font-semibold text-zinc-400 uppercase">
                    Key Name
                  ...
                  <th className="p-4 text-xs font-semibold text-zinc-400 uppercase text-right">
                    Actions
                  </th>
                </tr>
              </thead>
                  ...
                  >
                    ...
                    <td className="p-4 text-right">
                      <motion.button
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                        onClick={() => openAppDetails(key.appId)}
                        className="text-sm bg-zinc-800 hover:bg-zinc-700 px-3 py-1.5 rounded text-white transition-colors"
                      >
                        Manage
                      </motion.button>
                    </td>
  ...
  const renderContent = () => {
    ...
    if (activeTab === "integrations")
      return <IntegrationsView key="integrations" />;
  ...
  return (
    <div className="flex h-screen w-full bg-[#0B0C10] text-zinc-200 font-sans overflow-hidden">
      ...
      <div className="w-64 bg-[#111217] border-r border-zinc-800 flex flex-col h-full flex-shrink-0 z-[50]">
        ...
        <div className="px-4 pb-4 space-y-2 shrink-0">
              ...
              >
                ...
                >
                  <Plug
                    size={16}
                    className={
                      activeTab === "integrations" && !selectedAppId
                        ? "text-[#23b5b5]"
                        : "text-zinc-400"
                    }
                  />
                ...
                >
                  <Key
                    size={16}
                    className={
                      activeTab === "api_keys" && !selectedAppId
                        ? "text-[#23b5b5]"
                        : "text-zinc-400"
                    }
                  />
                ...
                >
                  <Settings
                    size={16}
                    className={
                      activeTab === "settings" && !selectedAppId
                        ? "text-[#23b5b5]"
                        : "text-zinc-400"
                    }
                  />
          ...
          >
            ...
            <ChevronDown
              size={14}
              className={`text-zinc-500 transition-transform duration-200 shrink-0 ${isUserMenuOpen ? "rotate-180" : ""}`}
            />
          </div>
        </div>
      </div>
        ...
        <header className="h-16 flex items-center justify-between px-8 border-b border-zinc-800 bg-[#111217]/80 backdrop-blur-md sticky top-0 z-10 shrink-0">
          <div className="flex items-center text-sm font-medium text-zinc-400">
            ...
            <span className="capitalize text-white">
              {activeTab === "store" ? "Store" : activeTab.replace("_", " ")}
            </span>
      ...
      <AnimatePresence>
        ...
        {lightboxOpen &&
          selectedApp &&
          (selectedApp.featureImages || []).length > 0 && (
            <Lightbox
              images={selectedApp.featureImages}
